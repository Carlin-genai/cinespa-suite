...
-- 3. Create the 4 required teams
INSERT INTO public.teams (name, description, org_id)
VALUES 
  ('Accounts', 'Financial management and accounting team', (SELECT org_id FROM public.profiles WHERE id = auth.uid() LIMIT 1)),
  ('Automation', 'Process automation and technical solutions team', (SELECT org_id FROM public.profiles WHERE id = auth.uid() LIMIT 1)),
  ('Cinespa 20', 'Cinema and spa management for location 20', (SELECT org_id FROM public.profiles WHERE id = auth.uid() LIMIT 1)),
  ('Home Theatre', 'Home entertainment systems and theatre setup team', (SELECT org_id FROM public.profiles WHERE id = auth.uid() LIMIT 1))
ON CONFLICT (name, org_id) DO NOTHING;

-- 4. Update task INSERT policies to allow team heads to assign tasks
...
-- 5. Update SELECT policy to allow proper visibility for all roles
