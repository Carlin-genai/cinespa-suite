-- Fix task creation and update flow with safe defaults and proper RLS

-- 1. Add unique constraint for teams (name, org_id) to support ON CONFLICT
ALTER TABLE public.teams ADD CONSTRAINT teams_name_org_id_unique UNIQUE (name, org_id);

-- 2. Update set_task_defaults function to handle all safe defaults
...
-- 3. Update RLS policies for unlimited task status updates
...
-- 4. Create the 4 required teams (using a simpler approach)
INSERT INTO public.teams (name, description, org_id)
SELECT 'Accounts', 'Financial management and accounting team', profile.org_id
FROM public.profiles profile WHERE profile.id = auth.uid() LIMIT 1
ON CONFLICT (name, org_id) DO NOTHING;

INSERT INTO public.teams (name, description, org_id)
SELECT 'Automation', 'Process automation and technical solutions team', profile.org_id
FROM public.profiles profile WHERE profile.id = auth.uid() LIMIT 1
ON CONFLICT (name, org_id) DO NOTHING;

INSERT INTO public.teams (name, description, org_id)
SELECT 'Cinespa 20', 'Cinema and spa management for location 20', profile.org_id
FROM public.profiles profile WHERE profile.id = auth.uid() LIMIT 1
ON CONFLICT (name, org_id) DO NOTHING;

INSERT INTO public.teams (name, description, org_id)
SELECT 'Home Theatre', 'Home entertainment systems and theatre setup team', profile.org_id
FROM public.profiles profile WHERE profile.id = auth.uid() LIMIT 1
ON CONFLICT (name, org_id) DO NOTHING;

-- 5. Update task INSERT policies to allow team heads to assign tasks
...
-- 6. Update SELECT policy to allow proper visibility for all roles
